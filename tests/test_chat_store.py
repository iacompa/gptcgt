...
from src.core.chat_store import ChatMessage, ChatStore, MessageRole
...
def _make_msg(role: MessageRole, content: str) -> ChatMessage:
    return ChatMessage(
        id=str(uuid.uuid4()), role=role, content=content, timestamp=datetime.now(timezone.utc)
    )
...
def test_add_message_persists(chat_store, chat_workspace):
    ...
    chat_store.add_message(msg.role, msg.content)
...
def test_search_across_sessions(chat_store):
    ...
    chat_store.current_session_id = "session1"
    chat_store.add_message(MessageRole.USER, "Find the hidden treasure")

    # Session 2
    chat_store.new_session()
    chat_store.current_session_id = "session2"
    chat_store.add_message(MessageRole.USER, "Nothing here")
...
def test_session_export_markdown(chat_store):
    ...
    chat_store.add_message(msg.role, msg.content, model_id="claude-3-5")
...
async def test_context_compactor_respects_budget():
    ...
    history = [
        _make_msg(MessageRole.USER, "A" * 500)
    ]  # Assuming 1 token = 4 chars, this is > 100 tokens
...
def test_atomic_write_survives_crash(chat_store, chat_workspace):
    ...
    try:
        chat_store.add_message(MessageRole.USER, "Crash me")
