...
def isolate_tracker():
    ...
    p = Path("/tmp/gptcgt_test_spending.json")
    if p.exists():
        p.unlink()
    yield
    if p.exists():
...
def test_task_cost_sums_models():
    ...
    m1 = ModelUsage(
        "m1", "M1", "prov", "role", input_tokens=1000, output_tokens=500, total_cost=0.01
    )
    m2 = ModelUsage(
        "m2", "M2", "prov", "role", input_tokens=2000, output_tokens=1000, total_cost=0.05
    )
    ...
    assert math.isclose(task.total_cost, 0.06)
...
def test_daily_spend_accumulates():
    ...
    assert math.isclose(today.total_cost, 0.06)
