...
def test_directory_prefix_collision(test_workspace):
    ws, root = test_workspace
    # If root is /tmp/project, an evil path is /tmp/project-evil/file.txt
    # This tests the "str.startswith" vulnerability
    evil_path = str(root) + "-evil/secret.txt"
    with pytest.raises(WorkspaceEscapeError):
        ws.validate_path(evil_path)
