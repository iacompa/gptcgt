...
from src.core.quality_tiers import TIER_CONFIGS, QualityTier, QualityTierManager
...
def test_light_tier_prefers_cheap_models():
    ...
    assert any(
        "mini" in m.lower() or "flash" in m.lower() or "deepseek" in m.lower() for m in prefs
    )
