...
export async function GET(request: NextRequest) {
    ...
    if (error) {
        return sameOriginRedirect(request, `/auth?error=${encodeURIComponent(error)}`);
    }

    if (!code) {
        return sameOriginRedirect(request, '/auth?error=missing_code');
    }
    ...
    if (state && !storedState && isDeviceState(state)) {
        try {
            ...
            return sameOriginRedirect(request, '/auth?error=device_callback_failed');
        }
    }
    ...
    if (!state || !storedState || state !== storedState) {
        ...
        const response = sameOriginRedirect(request, '/auth?error=invalid_state');
    ...
    try {
        ...
        if (!backendRes.ok) {
            ...
            return sameOriginRedirect(request, '/auth?error=sso_failed');
        }
        ...
        if (!email) {
            return sameOriginRedirect(request, '/auth?error=no_email');
        }
        ...
        const response = sameOriginRedirect(request, '/dashboard');
        ...
        return sameOriginRedirect(request, '/auth?error=callback_failed');
    }
}
