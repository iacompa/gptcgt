...
class XAIAgent(BaseAgent):
    ...
    async def chat_stream(self, messages: list[dict]) -> AsyncGenerator[AgentResponse, None]:
        api_key = self.config.api_key or KeyChainManager.get_key("XAI_API_KEY")
        if not api_key:
            yield AgentResponse(error="XAI_API_KEY not found in keychain.", is_streaming=False)
            return
