...
class CostSummaryWidget(Static):
    ...
    def render(self) -> str:
        ...
        line1 = f"─── Cost: ${self.breakdown.total_cost:.3f} │ {self.breakdown.credits_used} credits │ {in_k:.1f}K/{out_k:.1f}K tokens │ {self.breakdown.total_duration:.1f}s ───"
        
        models_summary = []
        for m in self.breakdown.model_usages:
            if m.was_refusal: continue
